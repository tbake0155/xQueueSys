...
int main(int argc, char** argv)
{
    ...
    // get the user args
    int args = argc -4;  // number of args passed
    char *child_args[args]; // C string array to hold args
    
    // store args into the array
    for(int i=0; i<args; i++)
    {
        child_args[i] = argv[i + args];
    }
...
static void xqueuesys_handler(int signum)
{
    if (signum == SIGUSR1)
    {
        ...
        kill(parent_pid, SIGUSR2); // alert xqueuesys
        kill(getpid(), SIGTERM); // harikari
    }
}
